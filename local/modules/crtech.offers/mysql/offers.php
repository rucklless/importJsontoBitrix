...
class CSaleOffers extends CAllSaleBasket
{
	/**
	...
	*/
	function Add($arFields)
	{
		...
		if (!$bFound)
		{
			...
			if ($isOrderConverted == 'N')
			{
				$arInsert = $DB->PrepareInsert("crtech_offers", $arFields);

				$strSql = "INSERT INTO crtech_offers(".$arInsert[0].", DATE_INSERT, DATE_UPDATE) VALUES(".$arInsert[1].", ".$DB->GetNowFunction().", ".$DB->GetNowFunction().")";
				...
				if ($boolProps)
				{
					foreach ($arFields["PROPS"] as &$prop)
					{
						if ('' != $prop["NAME"])
						{
							$arInsert = $DB->PrepareInsert("crtech_offers_props", $prop);

							$strSql = "INSERT INTO crtech_offers_props(BASKET_ID, ".$arInsert[0].") VALUES(".$ID.", ".$arInsert[1].")";
							$DB->Query($strSql, false, "File: ".__FILE__."<br>Line: ".__LINE__);
						}
					}
					if (isset($prop))
	...
	function Delete($ID)
	{
		...
		$DB->Query("DELETE FROM crtech_offers_props WHERE BASKET_ID = ".$ID, true);
		...
		$DB->Query("DELETE FROM crtech_offers WHERE ID = ".$ID, true);
	...
	function DeleteAll($FUSER_ID = 0, $bIncOrdered = false)
	{
		...
		while ($arBasket = $dbBasket->Fetch())
		{
			...
			else
			{
				...
				$DB->Query("DELETE FROM crtech_offers_props WHERE BASKET_ID = ".$arBasket["ID"], true);
				$DB->Query("DELETE FROM b_sale_store_barcode WHERE BASKET_ID = ".$arBasket["ID"], true);
				$DB->Query("DELETE FROM crtech_offers WHERE ID = ".$arBasket["ID"], true);
			}
		}
