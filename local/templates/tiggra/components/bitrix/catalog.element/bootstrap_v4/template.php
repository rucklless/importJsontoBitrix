...
$buyButtonClassName = in_array('BUY', $arParams['ADD_TO_BASKET_ACTION_PRIMARY']) ? '' : 'btn-link';
...
?>
	<div class="detail-page bx-catalog-element<?=$themeClass?>" id="<?=$itemIds['ID']?>" itemscope itemtype="http://schema.org/Product">
		...
		<div class="right">
				...
				?>
						...
						{
							?><div class="scu-section" id="<?=$itemIds['TREE_ID']?>"><?
										...
										if ($showOffersBlock)
										{
											?>
												...
												foreach ($arResult['SKU_PROPS'] as $skuProperty)
												{
													...
													?>
													<div class="scu-wrap" data-entity="sku-line-block">
														<div class="scu-title"><?=htmlspecialcharsEx($skuProperty['NAME'])?></div>
														<div class="scu-list">
															<ul>
																<?
																foreach ($skuProperty['VALUES'] as &$value)
																{
																	...
																	if ($skuProperty['SHOW_MODE'] === 'PICT')
																	{
																		if($skuProperty['CODE'] == 'COLOR_REF'){
																			foreach($arResult["OFFERS"] as $offer){
																				if($offer['PROPERTIES']['COLOR_REF']['VALUE'] == $value['XML_ID']){?>
																					<?if(isset($offer['DISPLAY_PROPERTIES']['MORE_PHOTO']['FILE_VALUE']['ID']))
																						$fileId = $offer['DISPLAY_PROPERTIES']['MORE_PHOTO']['FILE_VALUE']['ID'];
																					elseif (isset($offer['DISPLAY_PROPERTIES']['MORE_PHOTO']['FILE_VALUE'][0]['ID']))
																						$fileId = $offer['DISPLAY_PROPERTIES']['MORE_PHOTO']['FILE_VALUE'][0]['ID'];
																					else
																						$fileId = null;
																					?>
																					<?$tmp = CFile::ResizeImageGet($fileId, array('width'=>50, 'height'=>50), BX_RESIZE_IMAGE_PROPORTIONAL_ALT, true);
																					break;
																				}
																			}
																			?>
			...
			<div class="mb-3">
				...
				<div class="price-block">
					...
					while ($arItemsBasket = $dbBasketItems->Fetch()) {
						$allBasketItems[] = $arItemsBasket['PRODUCT_ID'];
					}
					foreach ($arResult["OFFERS"] as $arOffer) {
						$allItemOffersIds[] = $arOffer['ID'];
					}
					if(is_array($allBasketItems) && count($allBasketItems)>0){
						$IsSkusInBasket = array_intersect($allBasketItems, $allItemOffersIds);
					}
					?>
					<?if(empty($IsSkusInBasket)) {
						$basketBtnText = Loc::getMessage('CT_BCE_CATALOG_ADD');
						$basketBtnClass="";
						$icon = "fa-shopping-cart";

					} else {
						$basketBtnText = Loc::getMessage('CT_BCE_CATALOG_ADDED');
						$basketBtnClass="added";
						$icon = "fa-check";
					}?>
					<div class="add-cart" id="<?=$itemIds['BASKET_ACTIONS_ID']?>" style="display: <?=($actualItem['CAN_BUY'] ? '' : 'none')?>;">
						<?if ($showAddBtn)
							{
								?>
								<a class="buy-link temp-bg-button <?=$basketBtnClass?> <?=$showButtonClassName?>" id="<?=$itemIds['ADD_BASKET_LINK']?> detail_add_to_cart"
								   href="javascript:void(0);"><i class="fa <?=$icon?>"></i><span><?=$basketBtnText?></span></a>
								<?
							}
							...
							{
								?>
								<a class="buy-link temp-bg-button <?=$basketBtnClass?> <?=$buyButtonClassName?>"
								   id="<?=$itemIds['BUY_LINK']?>"
								   href="javascript:void(0);"><i class="fa <?=$icon?>"></i>
									<span><?=$basketBtnText?></span>
								</a>
								<?
						}?>
						<a class="del"
						   id="<?=$itemIds['DELETE_LINK']?>">
				...
				<div class="<?=($showBlockWithOffersAndProps ? "col-lg-7" : "col-lg"); ?>">
					<div class="product-item-detail-pay-block-normalize">
						<?
	...
	<div class="bottom">
		<ul class="nav detail-tab" id="detailTab" role="tablist">
