	...
	window.JCCatalogElement.prototype = {
		...
		init: function()
		{
			...
			if (this.config.showPrice)
			{
				...
				this.obBuyDelBtn = BX(this.visual.DELETE_LINK);
			}
		...
		initBasketData: function()
		{
			if (this.params.BASKET && typeof this.params.BASKET === 'object')
			{
				...
				if (this.params.BASKET.DEL_URL_TEMPLATE)
				{
		...
		changeInfo: function()
		{
			...
			/**
			 * Запускаем после инициализации setInBasket, дабы не плодить запросы ибо повторяется
			 * */
			if(this.basketData.checkSetInBasket){
		...
		initBasketUrl: function()
		{
			if(this.basketMode === 'ADD')
				...
				this.basketUrl = this.basketData.del_url
			else
		...
		crt_Add2BasketAnimation:function(){
				var button = $('.buy-link');
				var cart = $('.basket-fly .basket');
		...
		deleteBasket: function()
		{
			...
			BX.ajax.post(
				...
				BX.delegate(function (data){
					this.setInBasket(this.offers[this.offerNum].ID, 'del');
				}, this)
			);
		},
		deleteBasketItem: function(){
			this.crtRefreshCart('del');
		...
		setInBasket: function(data, action){
			if(action){
				var indx = this.basketData.in_basket.indexOf(data)
				if(indx != -1 && indx >=0){
					if(action == 'del'){
						this.basketData.in_basket.splice(indx, 1);
					}
				}else{
					if(action == 'add'){
						this.basketData.in_basket.push(data);
					}
				}
			}else{
				this.basketData.in_basket = data;
			}
			if (!this.basketData.checkSetInBasket){
				this.basketData.checkSetInBasket = true;
			}
			this.checkButtonBasket();
		},
		checkButtonBasket: function(){
			...
			if(this.basketData.in_basket.indexOf(this.offers[this.offerNum].ID) >= 0){
				for(var l=0; l<buyLinks.length;l++){
					...
					buyLinks[l].querySelector('span').innerText = "В корзине";
				}
			}else{
				for(var l=0; l<buyLinks.length;l++){
					buyLinks[l].classList.remove('added');
					buyLinks[l].querySelector('span').innerText = "В корзину";
				}
			}
			this.crtRefreshCart();
		},
		...
		checkBasket: function(){
			BX.ajax({
				...
				onsuccess: function (data){
					...
					if(data.arResult.length>0){
						for(var i = 0; i<data.arResult.length; i++){
							this.basketData.list[i] = data.arResult[i].PRODUCT_ID
						}
						//console.log(this.basketData.list);
					}*/
				}
			});
		},
